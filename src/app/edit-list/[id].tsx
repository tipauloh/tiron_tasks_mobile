...
export default function EditListScreen() {
  ...
  useEffect(() => {
    if (list && !hydrated) {
      setName(list.name);
      setSelectedColor(list.color ?? Colors.primary);
  ...
  const isDirty =
    name.trim() !== (list?.name ?? '') ||
    selectedColor !== (list?.color ?? Colors.primary) ||
    selectedIcon !== (list?.icon ?? null);

  const canSave = name.trim().length > 0 && isDirty;
  const isLoading = updateList.isPending;
    ...
    return (
      <SafeAreaView style={[styles.safe, { backgroundColor: theme.colors.background }]} edges={['top', 'bottom']}>
        <View style={[styles.header, { borderBottomColor: theme.colors.border }]}>
          ...
          <Text variant="callout" weight="semibold">Editar lista</Text>
          <View style={{ width: 48 }} />
        </View>
    ...
    <KeyboardAvoidingView style={{ flex: 1 }} behavior={Platform.OS === 'ios' ? 'padding' : 'height'}>
      <SafeAreaView style={[styles.safe, { backgroundColor: theme.colors.background }]} edges={['top', 'bottom']}>
        <View style={[styles.header, { borderBottomColor: theme.colors.border }]}>
          ...
          <Text variant="callout" weight="semibold">Editar lista</Text>
          <TouchableOpacity onPress={handleSave} disabled={!canSave || isLoading} hitSlop={{ top: 8, bottom: 8, left: 8, right: 8 }}>
            <Text variant="body" weight="semibold" style={{ color: canSave && !isLoading ? Colors.primary : theme.colors.textTertiary }}>Salvar</Text>
          </TouchableOpacity>
        </View>
        ...
        <ScrollView contentContainerStyle={styles.scroll} keyboardDismissMode="interactive" showsVerticalScrollIndicator={false}>
          ...
          <View style={[styles.section, styles.deleteSection]}>
            <TouchableOpacity
              ...
              activeOpacity={0.7}
            >
              <Text variant="body" style={{ color: Colors.danger, fontWeight: '600' }}>
                {archiveList.isPending ? 'Excluindo...' : 'Excluir lista'}
              </Text>
            </TouchableOpacity>
          </View>
        </ScrollView>
